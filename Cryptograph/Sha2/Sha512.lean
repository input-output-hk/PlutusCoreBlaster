...
def padMessage (x : List UInt8) : List UInt8 :=
  ...
  let padLen := (112 - (len + 1) % 128) % 128
  let x := x ++ List.replicate padLen 0

  -- Append 128-bit big-endian length (upper 64 bits = 0)
  let x := x ++ List.replicate 8 0
  let x := x ++ UInt64.toUInt8BE bitLen.toUInt64
