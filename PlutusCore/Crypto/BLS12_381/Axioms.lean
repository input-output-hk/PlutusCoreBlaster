...
/-! ## Axiomatisation for the PlutusCore BLS12_381 builtin functions. -/
...
/- ### The domain of these axioms: the order-r subgroup

   `InG1 a` reads: `a` lies in the order-`r` subgroup generated by `g1`.  Most axioms below
   are guarded by it, because they are false of an arbitrary point of the curve.

   It is `opaque` rather than `def` so that it does not *unfold*: every consumer wants an
   uninterpreted predicate here, and `InG1_def` is the only way in or out.  The intended
   definition is right there in the body, which is also the witness that a model exists, so
   the one axiom per group this costs is definitional rather than an assumption about the
   curve.  The kernel-side proofs never need the definition anyway -- only the closure
   lemmas below, which are re-proved through `InG1_def` at no cost.

   Do not restate `InG1_def` in a module that runs `blaster`: a same-module `Prop` axiom is
   harvested into *every* query in that module (measured in `Tests/AxiomsBlasterProbe.lean`),
   and this one mentions `n * g1`, whose scalar action bottoms out in a `partial def` that
   the SMT translation refuses -- so it would hard-error goals that never mention `InG1` at
   all. -/
...
/- The subgroups are closed under the addition, negation and scalar-multiplication operations. -/
...
/- The generator's order divides r.  Derived rather than assumed, and stated below where
   `g1_dlog_emod_eq_zero_iff` is available. -/
...
/- ### Pairing target `G_T`, as ℤ/r
   ...
   [0, r). This is the same collapse `g1_dlog`/`g2_dlog` perform for G1 and G2, and it also
   keeps the sort translatable -- an `axiom GT : Type` is uninterpreted, which the SMT
...
/- `gtGen` has order exactly r: what the ℤ/r model says, and the only structural fact about
   `G_T` that anything below uses. -/
...
axiom g1_uncompress_compress :
  InG1 a₁ → bls12_381_G1_uncompress (bls12_381_G1_compress a₁) = Except.ok a₁
axiom g1_compress_uncompress :
  bls12_381_G1_uncompress b = Except.ok a₁ → bls12_381_G1_compress a₁ = b
axiom g1_uncompress_subgroup : bls12_381_G1_uncompress b = Except.ok a₁ → InG1 a₁

axiom g2_uncompress_compress :
  InG2 a₂ → bls12_381_G2_uncompress (bls12_381_G2_compress a₂) = Except.ok a₂
axiom g2_compress_uncompress :
  bls12_381_G2_uncompress b = Except.ok a₂ → bls12_381_G2_compress a₂ = b
...
/- The converse needs both Miller loops to be genuine -- i.e. nonzero subgroup arguments,
   which is what `millerLoop_ok` turns into `MlOk`. -/
...
theorem g1_dlog_add :
  ...
  → g1_dlog (a₁ + b₁) = (g1_dlog a₁ + g1_dlog b₁) % r :=
    by
      intro ha hb
      conv => lhs; rw [g1_cyclic a₁ ha, g1_cyclic b₁ hb,
                       ←g1_scalarMul_add_scalar _ _ _ InG1_gen, g1_dlog_scalarMul]
...
theorem g2_dlog_add :
  ...
  → g2_dlog (a₂ + b₂) = (g2_dlog a₂ + g2_dlog b₂) % r :=
    by
      intro ha hb
      conv => lhs; rw [g2_cyclic a₂ ha, g2_cyclic b₂ hb,
                       ←g2_scalarMul_add_scalar _ _ _ InG2_gen, g2_dlog_scalarMul]
...
theorem g1_dlog_emod_eq_zero_iff :
  InG1 a₁ → (g1_dlog a₁ % r = 0 ↔ a₁ = 0) :=
    by
      intro ha
      constructor <;> intro h
      · have h₀ : a₁ = (g1_dlog a₁ % r) * g1 := by
          rw [g1_scalarMul_mod g1 _ InG1_gen]; exact g1_cyclic a₁ ha
...
theorem g2_dlog_emod_eq_zero_iff :
  InG2 a₂ → (g2_dlog a₂ % r = 0 ↔ a₂ = 0) :=
    by
      intro ha
      constructor <;> intro h
      · have h₀ : a₂ = (g2_dlog a₂ % r) * g2 := by
          rw [g2_scalarMul_mod g2 _ InG2_gen]; exact g2_cyclic a₂ ha
        rw [h₀, h, g2_scalarMul_zero]
      · rw [h, g2_dlog_zero]
        exact Int.zero_emod r

/- ### The generator's order divides r

   Derived, not assumed and not computed: `g1_dlog_scalarMul` sends `r * g1` to `r % r = 0`,
   and `g1_dlog_emod_eq_zero_iff` reads a zero discrete log back as the zero point.  The one
   thing no derivation can establish -- that the `r` above really is the order of the
   generator of the curve `Cryptograph` implements -- is checked numerically in
   `Tests/AxiomsValidate.lean`, which is where this file keeps evaluation. -/

theorem g1_order : r * g1 = 0 :=
  (g1_dlog_emod_eq_zero_iff (r * g1) (InG1_smul_gen r)).mp
    (by rw [g1_dlog_scalarMul, Int.emod_self, Int.zero_emod])

theorem g2_order : r * g2 = 0 :=
  (g2_dlog_emod_eq_zero_iff (r * g2) (InG2_smul_gen r)).mp
    (by rw [g2_dlog_scalarMul, Int.emod_self, Int.zero_emod])
...
/- These three are facts about ℤ/r and nothing else: under the model they are arithmetic,
   and depend on no curve axiom. -/
  ...
  → m % r = 0 ∨ n % r = 0 :=
    by
      ...
      have hE : e (m * g1) (n * g2) = gtOne := by
        rw [e_dlog _ _ (InG1_smul_gen m) (InG2_smul_gen n),
            g1_dlog_scalarMul, g2_dlog_scalarMul, gtOne_eq_gtPow_zero]
        exact (gtPow_inj_mod _ _).mpr (by rw [← Int.mul_emod, h, Int.zero_emod])
      have hnE : m * g1 = 0 ∨ n * g2 = 0 :=
        (e_nondegen _ _ (InG1_smul_gen m) (InG2_smul_gen n)).mp hE
      match hnE with
...
/- ### The axioms as bundles
   ...
   Every Prop-typed axiom of this file except the two definitional ones, `InG1_def` and
   `InG2_def`, grouped by the sections above as closed `Prop`s and paired with the witness
   that discharges it.  A consumer states `theorem foo_of (h : SomeAlg) ... := by blaster`
   and applies it to the witness, so the axioms are *used* rather than restated and the
   footprint stays honest.

   Alongside the seven, three kinds of companion bundle:

   * `G1AddNegAlg`, `G2AddNegAlg` and `G1SmulAlg` are named *slices* of `G1Alg`/`G2Alg`.
     Twelve conjuncts at once leave the solver `Undetermined`, so a goal wants the two or
     three laws it actually needs; these are the slices that have turned out to be worth a
     name.
   * `PairingSerdeAlg` collects the five bundles whose conclusions mention no G1/G2 point
     operation, which is the half that needs nothing of the consumer.
   * `DlogFacts` is derived rather than assumed -- every conjunct is a theorem proved
     above -- and so is deliberately not part of `AllAlg`.

   `G1Alg` and `G2Alg` ask one step of the consumer: their conclusions are equations in the
   `Cryptograph` `+`/`*`, which bottom out in a `partial def` the SMT backend refuses.
   Marking the four `Cryptograph` `Point` instances `irreducible` in the *consuming* module
   leaves those operations uninterpreted and both bundles then translate, at no cost to the
   trust base -- an attribute is not a proof step.  The seal has to go in the consuming
   module because it changes every `blaster` call after it;
   `Tests/AxiomsBlasterProbe.lean` measures the whole arrangement and carries the vacuity
   guards it needs. -/
...
/-- The G1 group: the scalar action at 0, 1 and -1, the abelian group laws, the ℤ/r-module laws
    ...
    then a slice at a time rather than whole -- see `Tests/AxiomsBlasterProbe.lean` and the
    named slices below. -/
def G1Alg : Prop :=
...
/-- The additive slice of `G1Alg`: commutativity, the identity, and inverses.  Enough for the
    abelian-group rewriting a goal usually needs, and small enough for the solver. -/
...
/-- The scalar-action slice of `G1Alg`, including reduction by the 255-bit modulus `r` --
    which is not what makes the full bundle stall: these three leave the solver `Valid`. -/
def G1SmulAlg : Prop :=
  (∀ (p : BLS12_381_G1_Element), (1 : Int) * p = p) ∧
  (∀ (p q : BLS12_381_G1_Element), p + q = q + p) ∧
  (∀ (p : BLS12_381_G1_Element) (i : Int), InG1 p → (i % r) * p = i * p)

theorem g1SmulAlg : G1SmulAlg := ⟨g1_scalarMul_one, g1_add_comm, g1_scalarMul_mod⟩
...
/-- `G1AddNegAlg` for G2, statement for statement.  The one seal covers both groups. -/
def G2AddNegAlg : Prop :=
  (∀ (q r' : BLS12_381_G2_Element), q + r' = r' + q) ∧
  (∀ (q : BLS12_381_G2_Element), q + 0 = q) ∧
  (∀ (q : BLS12_381_G2_Element), InG2 q → q + -q = 0)
...
/-- Commutativity and associativity of `mulMlResult`, which are exact -- `mulMlResult` is
    Fq12 multiplication, with no domain restriction. -/
def MlAlg : Prop :=
...
/-- The soundness slice of `BridgeAlg`: `finalVerify` decides equality of `pi`, and `pi`
    after a Miller loop is the pairing.  These two are what carries a `finalVerify` result
    back to a statement about `e`. -/
def PairingBridgeAlg : Prop :=
  (∀ (x y : BLS12_381_MlResult), bls12_381_finalVerify x y = true → pi x = pi y) ∧
  (∀ (p : BLS12_381_G1_Element) (q : BLS12_381_G2_Element),
     pi (bls12_381_millerLoop p q) = e p q)

theorem pairingBridgeAlg : PairingBridgeAlg := ⟨finalVerify_sound, pi_millerLoop⟩
...
/-- Eight *derived* facts that translate -- the point-free corner of the ℤ/r layer. Not part
    of `AllAlg`, and deliberately not part of `PairingSerdeAlg`: every conjunct is a theorem
    ...
    operation -- eleven axioms a `PairingSerdeAlg`-only proof would not name.
...
/-- The pairing and serialisation layer: the 18 axioms whose conclusions mention no G1/G2
    point *operation*, and which therefore need nothing of the consumer -- `InG1`/`InG2`
    guards are fine, being uninterpreted predicates.  That leaves only `G1Alg` and `G2Alg`
    outside, and those are reachable too in a module that seals the point instances.

    Prefer the narrowest bundle a goal needs.  `#print axioms` reports what the *premise*
    mentions rather than what the solver used, so a proof off this one names all 18 whether
    it needed them or not. -/
def PairingSerdeAlg : Prop := MlAlg ∧ MlOkAlg ∧ BridgeAlg ∧ PairingAlg ∧ SerdeAlg

theorem pairingSerdeAlg : PairingSerdeAlg :=
  ⟨mlAlg, mlOkAlg, bridgeAlg, pairingAlg, serdeAlg⟩

/-- Every Prop-typed axiom of this module except `InG1_def` and `InG2_def`: 42 of the 44.
    The two exclusions are the definitional unfoldings of the `opaque` subgroup predicates,
    which no consumer should restate -- see the domain section at the top of the file.

    Exhaustive by construction otherwise: adding an axiom above without extending a bundle
    here leaves `#print axioms allAlg` an incomplete picture of what this file assumes, so
    extend it.  The five data axioms -- `e`, `pi`, `MlOk`, `g1_dlog`, `g2_dlog` -- appear in
    that footprint anyway, since the bundles mention them.

    The conjunct order matches `PairingSerdeAlg`, so the two share a destructuring. -/
def AllAlg : Prop :=
  G1Alg ∧ G2Alg ∧ MlAlg ∧ MlOkAlg ∧ BridgeAlg ∧ PairingAlg ∧ SerdeAlg
